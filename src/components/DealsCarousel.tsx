...
/**
 * Generic horizontally-scrolling, auto-rotating teaser strip — shared by
 * the limited-time deals section and the quantity/wholesale deals
 * section, since both need the exact same (fairly intricate, bug-fixed
 * over several rounds) carousel mechanics and only differ in what data
 * they show and where "show more" goes.
 *
 ...
 */
export function DealsCarousel<T>({
  title,
  showMoreHref,
  showMoreLabel,
  items,
  ...
  useEffect(() => {
    ...
    cardRatiosRef.current = items.map(() => 0);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      goTo((activeIndexRef.current + 1) % items.length);
    }, AUTO_ADVANCE_MS);
      ...
      >
        {items.map((item, index) => (
          <div
            ...
            // active one scaling up beyond it. Scaling up made the
            // enlarged card bigger than the space the scroller had
            // reserved for it, and overflow-x-auto clips anything beyond
            // that — cropping the image. Shrinking never needs extra
            // space, so nothing can ever clip.
